...
public class UpdateUserUserOwnerJobPostingStepApplicationInProgressService implements UpdateUserUserOwnerJobPostingStepApplicationInProgressUseCase {
    ...
    public void execute(UUID accountId, Long userOwnerJobPostingId) {
        ...
        Notification userNotification = notificationService.createNotification(EKafkaStatus.USER_RESULT.getMessage(), userOwnerJobPosting);
        Notification ownerNotification = notificationService.createNotification(EKafkaStatus.OWNER_COMPLAINT.getMessage(), userOwnerJobPosting);
